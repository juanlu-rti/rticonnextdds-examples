...
#include "dds/dds.hpp"
#include "KyotoCabinetReader.hpp"
 ...
 */
RTI_RECORDING_STORAGE_READER_CREATE_DEF(KyotoCabinetReader);

KyotoCabinetReader::KyotoCabinetReader(
...
KyotoCabinetReader::create_stream_info_reader(
        const rti::routing::PropertySet&)
{
    return new KyotoCabinetStreamInfoReader(&info_file_);
}

void KyotoCabinetReader::delete_stream_info_reader(
...
KyotoCabinetReader::create_stream_reader(
        const rti::routing::StreamInfo&,
        const rti::routing::PropertySet&)
{
    return new KyotoCabinetStreamReader(&data_file_);
}

void KyotoCabinetReader::delete_stream_reader(
 ...
 * the given end time.
 */
KyotoCabinetStreamReader::KyotoCabinetStreamReader(std::ifstream *data_file) :
...
KyotoCabinetStreamReader::~KyotoCabinetStreamReader()
{
}

bool KyotoCabinetStreamReader::read_sample()
{
...
void KyotoCabinetStreamReader::read(
...
void KyotoCabinetStreamReader::return_loan(
        std::vector<dds::core::xtypes::DynamicData *>& sample_seq,
        std::vector<dds::sub::SampleInfo *>& info_seq)
{
...
bool KyotoCabinetStreamReader::finished()
{
    if (data_file_->eof()) {
        return true;
    } else {
...
void KyotoCabinetStreamReader::reset()
{
    data_file_->clear();
    data_file_->seekg(0);
    /* read-ahead, EOF is not an error */
...
KyotoCabinetStreamInfoReader::KyotoCabinetStreamInfoReader(
 ...
 */
void KyotoCabinetStreamInfoReader::read(
...
void KyotoCabinetStreamInfoReader::return_loan(
            std::vector<rti::routing::StreamInfo *>& sample_seq)
{
    sample_seq.clear();
}
 ...
 * '.info' file.
 */
int64_t KyotoCabinetStreamInfoReader::service_start_time()
{
    std::string line;
    int64_t timestamp = 0;
 ...
 * '.info' file.
 */
int64_t KyotoCabinetStreamInfoReader::service_stop_time()
{
    std::string line;
    int64_t timestamp = 0;
...
bool KyotoCabinetStreamInfoReader::finished()
{
    return stream_info_taken_;
}

void KyotoCabinetStreamInfoReader::reset()
{
    stream_info_taken_ = false;
}
